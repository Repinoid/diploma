...
func LoginUser(rwr http.ResponseWriter, req *http.Request) {
	...
	err = securitate.Interbase.CheckUserPassword(req.Context(), logos.UserName, logos.Password)
	if err != nil {
	...
	err = securitate.Interbase.UpdateToken(req.Context(), logos.UserName, Token)
	if err != nil {
		rwr.WriteHeader(http.StatusInternalServerError) //500 — внутренняя ошибка сервера.
		fmt.Fprintf(rwr, `{"status":"StatusInternalServerError"}`)
